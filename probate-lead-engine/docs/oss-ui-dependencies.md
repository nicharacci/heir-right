...
| `ag-grid-community` | `36.0.0` | MIT | Community edition only for Estates, Queue, and Admin/audit datasets | Bundled into `/assets/app.js` only when an approved grid feature registers it |
...
`ag-grid-enterprise`, Enterprise modules, integrated charts, Excel export, server-side row model, master/detail, row grouping, and sparklines are forbidden. The build and contract test reject Enterprise package references and known runtime asset CDNs in source or shipped entry assets.

const { requireApiAdmin, requireApiAuth, readJsonBody, sendJson } = require("../api/_shared");
const { effectiveSession } = require("../api/auth/_shared");
...
const PDF_EXTRACTION_RUNTIME_ROOT = join(__dirname, "../runtime-assets");
const PDFJS_STANDARD_FONT_DATA_URL = `${join(PDF_EXTRACTION_RUNTIME_ROOT, "pdfjs-dist", "standard_fonts")}${sep}`;
const PDFJS_MODULE_URL = pathToFileURL(join(PDF_EXTRACTION_RUNTIME_ROOT, "pdfjs-dist", "legacy", "build", "pdf.mjs")).href;
const PDFJS_WORKER_MODULE_URL = pathToFileURL(join(PDF_EXTRACTION_RUNTIME_ROOT, "pdfjs-dist", "legacy", "build", "pdf.worker.mjs")).href;
const DOM_MATRIX_MODULE_PATH = join(PDF_EXTRACTION_RUNTIME_ROOT, "@napi-rs", "canvas", "geometry.js");
...
async function extractPdf(bytes) {
  const { DOMMatrix } = require(DOM_MATRIX_MODULE_PATH);

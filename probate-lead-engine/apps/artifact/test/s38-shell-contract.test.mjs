...
function assertStaticContracts() {
  ...
  const beuiRuntimeCss = read("src/features/beui-runtime/runtime.css");
  ...
  assert.match(shellCss, /body\[data-s38-shell="case-journey"\] button:not\(:where\([\s\S]*\[role="tab"\][\s\S]*\[role="menuitem"\][\s\S]*\.hr-beui-table button[\s\S]*background:\s*var\(--hr-action-surface\)[\s\S]*border-radius:\s*var\(--hr-radius-chip\)/, "approved controls must share the tokenized graphite chip without leaking into tabs, menus, or BeUI rails");
  ...
  assert.match(shellCss, /#workspace\[data-s38-shell="case-journey"\] \.nav-item\.is-active \{[\s\S]*background:\s*var\(--hr-pearl-selected\)[\s\S]*border:\s*0;[\s\S]*box-shadow:\s*none/, "the selected primary navigation item must retain one borderless pearl-tinted bubble without decorative rings");
  ...
  assert.match(shellCss, /\.nav-icon::before \{[\s\S]*display:\s*none !important/, "the S38 shell must suppress the legacy radial navigation bloom");
  assert.match(shellCss, /\.nav-item:not\(\.is-active\):not\(:disabled\):is\(:hover, :focus-visible, :active\) \.nav-icon \{[\s\S]*drop-shadow\(0 0\.0625rem 0\.125rem rgb\(44 91 141 \/ 24%\)\)/, "navigation feedback must use the restrained directional deep-navy glyph shadow");
  ...
  assert.match(shellCss, /\.user-strip \.avatar \{[\s\S]*width:\s*2rem;[\s\S]*height:\s*2rem;[\s\S]*overflow:\s*hidden;[\s\S]*background-size:\s*contain;[\s\S]*border-radius:\s*50%/, "the account photo must remain a fitted, clipped circular avatar in both sidebar modes");
  ...
  assert.match(tokensCss, /:root \{[\s\S]*--hr-canvas:\s*#05070a;[\s\S]*--hr-chrome:\s*#080a0e;[\s\S]*--hr-surface:\s*#0d1015;[\s\S]*--hr-accent:\s*#1d4f85;[\s\S]*--hr-accent-strong:\s*#5f87b2;[\s\S]*--hr-icon-glow:\s*rgb\(44 91 141 \/ 42%\)/, "graphite mode must use the approved near-black surface ladder and deep-navy interaction tokens");
  assert.match(tokensCss, /--hr-action-pearl:\s*radial-gradient\([\s\S]*ellipse 90% 180% at 50% -72%[\s\S]*#060a10 100%[\s\S]*--hr-action-pearl-hover:\s*radial-gradient\([\s\S]*--hr-action-pearl-active:\s*radial-gradient\([\s\S]*--hr-pearl-selected:\s*radial-gradient\(/, "the source tokens must provide the restrained top-lit pearl material and its interactive states");
  assert.match(tokensCss, /:root\[data-theme="cream"\] \{[\s\S]*--hr-surface-selected:\s*color-mix\(in srgb, var\(--hr-accent\) 12%, var\(--hr-surface\)\);[\s\S]*--hr-accent:\s*#194c80;[\s\S]*--hr-accent-strong:\s*#123a65;[\s\S]*--hr-action-border:\s*rgb\(25 76 128 \/ 62%\)/, "cream mode must retain its light surfaces while using the same deeper HeirRight navy");
  assert.match(shellCss, /\.shell-primary-command \{[\s\S]*background:\s*var\(--hr-action-pearl\)[\s\S]*\.shell-primary-command:is\(:hover, :focus-visible\) \{[\s\S]*background:\s*var\(--hr-action-pearl-hover\)[\s\S]*\.shell-primary-command:active \{[\s\S]*background:\s*var\(--hr-action-pearl-active\)/, "the shell's true primary command must own the pearl material across interaction states");
  assert.match(docPrepCss, /\.hr-docprep-flow-switch button\[aria-selected="true"\] \{[\s\S]*background:\s*var\(--hr-action-pearl\)[\s\S]*border:\s*1px solid var\(--hr-action-border\)[\s\S]*box-shadow:\s*inset 0 1px 0 var\(--hr-action-highlight\), 0 2px 3px rgb\(0 0 0 \/ 18%\)[\s\S]*\.hr-docprep-flow-switch button\[aria-selected="true"\]:is\(:hover, :focus-visible\) \{[\s\S]*background:\s*var\(--hr-action-pearl-hover\)[\s\S]*border-color:\s*var\(--hr-action-border-strong\)[\s\S]*\.hr-docprep-flow-switch button\[aria-selected="true"\]:active \{[\s\S]*background:\s*var\(--hr-action-pearl-active\)/, "the active Document Prep flow must share Continue Review's pearl material and interaction states");
  assert.match(shellCss, /\.journey-action-primary,[\s\S]*\.is-active\):is\(:hover, :focus-visible\) \{[\s\S]*background:\s*var\(--hr-action-pearl-hover\)[\s\S]*\.journey-action-primary,[\s\S]*\.is-active\):active \{[\s\S]*background:\s*var\(--hr-action-pearl-active\)/, "shared recommended actions must preserve distinct hover, focus, and pressed pearl states");
  assert.match(shellCss, /wa-button\[variant="brand"\]\[appearance="filled"\]::part\(base\) \{[\s\S]*background:\s*var\(--hr-action-pearl\)[\s\S]*wa-button\[variant="brand"\]\[appearance="filled"\]:is\(:hover, :focus-visible\)::part\(base\) \{[\s\S]*background:\s*var\(--hr-action-pearl-hover\)/, "filled brand actions must use the pearl material without changing ordinary Web Awesome controls");
  assert.match(compatCss, /\.btn\.primary,[\s\S]*\.source-pull-button,[\s\S]*\.command-form \.btn\.primary \{[\s\S]*background:\s*var\(--hr-action-pearl\)[\s\S]*border-color:\s*var\(--hr-action-border-strong\)/, "legacy primary and recommended source actions must inherit the pearl material from the semantic bridge");
  ...
  assert.doesNotMatch(gridsCss, /\.ag-/, "operational rails must not retain AG Grid styling after the BeUI migration");
  assert.match(gridsCss, /@media \(max-width: 620px\)[\s\S]*\.hr-estates-grid-view \{[\s\S]*padding-block-end:\s*calc\(var\(--s38-command-row-height\) \+ var\(--hr-space-4\)\)[\s\S]*\.hr-beui-rail-host \{[\s\S]*min-height:\s*520px;/, "mobile Estates must reserve a scroll tail and a usable BeUI rail above the always-visible command chip");
  assert.match(beuiRuntimeCss, /\.hr-beui-table \{[\s\S]*border:\s*0 !important;[\s\S]*border-radius:\s*0 !important;[\s\S]*background:\s*transparent !important;/, "operational BeUI rails must inherit the existing workbench surface instead of drawing a second table shell");
  assert.match(beuiRuntimeCss, /\.hr-beui-table tr\[data-selected="true"\] \{[\s\S]*background:\s*var\(--hr-surface-selected\) !important;/, "operational BeUI selections must keep the shared selected-state treatment");
  assert.match(gridsCss, /\.hr-grid-row-action \{[\s\S]*background:\s*var\(--hr-action-surface\);[\s\S]*border:\s*1px solid[\s\S]*border-radius:\s*var\(--hr-radius-chip\);[\s\S]*box-shadow:\s*inset 0 1px 0 var\(--hr-action-highlight\)/, "Queue's labeled Remove action must use the shared graphite chip material");
  ...
  assert.match(railHost, /defaultWidth \|\| 480/);
  assert.match(railHost, /minWidth \|\| 480/);
  assert.match(railHost, /maxWidth \|\| 552/);
  assert.match(journeyRailSource, /minWidth:\s*480,[\s\S]*maxWidth:\s*552,[\s\S]*defaultWidth:\s*480/, "Case Journey must share the reviewed desktop rail bounds");
  assert.match(docPrepRailSource, /minWidth:\s*480,[\s\S]*defaultWidth:\s*480,[\s\S]*maxWidth:\s*552/, "Document Prep must share the reviewed desktop rail bounds");
...
async function assertExecutableContracts() {
  ...
  assert.match(markup, /Estates Exported/);
  assert.match(markup, /7d.*14d.*30d/);
  assert.match(markup, /File history[^]*Estate Search/);

...
function assertStaticContracts() {
  ...
  assert.match(shellCss, /\.nav-item:not\(\.is-active\):focus-visible \{[\s\S]*outline:\s*2px solid transparent !important;[\s\S]*@media \(forced-colors: active\)[\s\S]*\.nav-item:not\(\.is-active\):focus-visible,[\s\S]*outline-color:\s*ButtonText !important/, "collapsed unselected navigation must use glyph glow for normal keyboard focus and recover an outline in forced-colors mode");
  ...
  assert.match(shellCss, /@media \(max-width: 1120px\)[\s\S]*body\[data-s38-shell="case-journey"\] \.shell-primary-command \{[\s\S]*background:\s*transparent !important[\s\S]*border:\s*0 !important[\s\S]*box-shadow:\s*none !important[\s\S]*\.shell-primary-command:not\(:disabled\):is\(:hover, :focus-visible, :active\)[\s\S]*drop-shadow\(0 0 0\.3rem var\(--hr-icon-glow\)\)[\s\S]*\.shell-primary-command:disabled[\s\S]*filter:\s*none !important[\s\S]*@media \(forced-colors: active\)[\s\S]*outline-color:\s*ButtonText !important/, "the compact glyph-only primary command must not regain the desktop plate while pressed, disabled, or keyboard-focused at any collapsed-label breakpoint");
  ...
  assert.match(shellCss, /:focus-visible \{[\s\S]*outline:\s*2px solid transparent !important;[\s\S]*@media \(forced-colors: active\)[\s\S]*outline-color:\s*ButtonText !important/, "icon-only keyboard focus must remain visible in forced-colors mode without a normal-mode square outline");

...
assert.match(source, /function verifiedSourceCaptureResult\(row = selectedRow\(\)\)[\s\S]*sourceRun\.persistence\?\.stored !== true[\s\S]*sourceRun\.persistence\?\.readbackStatus !== "verified"[\s\S]*!Array\.isArray\(capture\.sourceFacts\)/, "Only a complete shared-readback source capture may bypass a new public-source run.");

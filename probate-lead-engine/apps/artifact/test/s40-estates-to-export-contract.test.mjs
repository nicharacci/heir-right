...
const communityGrid = read("src/features/data-grid/community-grid.js");
...
assert.match(communityGrid, /function edgeResizeGuardedColumnDefs/);
assert.match(communityGrid, /resizable: column\?\.resizable === false \? false : index > 0 && index < lastIndex/);
...
assert.match(shellController, /shell-queue-table[^>]*data-hr-table-edge-resize=\\"guarded\\"/);
...
assert.match(legacy, /function csvImportItem/);
assert.match(legacy, /crmImportCsvFile/);
assert.match(legacy, /line\.includes\(";"\)/);
assert.match(legacy, /crmImportMaxFileBytes/);
assert.match(legacy, /FileReader/);
assert.match(legacy, /data-crm-upload-dropzone/);
...
assert.match(legacy, /async function submitBatchCrmImport/);
assert.match(legacy, /slice\(0, 200\)/);
...
assert.match(compatCss, /#crmImportSingle > span[\s\S]*margin-bottom: 4px/);
assert.match(compatCss, /#crmImportBatchToggle[\s\S]*margin-top: 2px/);

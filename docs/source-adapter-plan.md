...
| Miami-Dade Tax Collector | Guarded listing-page receipt client implemented for explicit receipt links, supplied listing HTML, direct listing URLs, configured listing URL templates, `TAX_COLLECTOR_BROWSER_WORKFLOW_URL`, and direct Browserbase Function invocation through `TAX_COLLECTOR_BROWSERBASE_FUNCTION_ID`; mocked Browserbase route proof maps function output into receipt/payer/tax facts; public GovHub entry still needs real Browserbase proof | folio, address, owner, listing page URL, browser workflow/function response | acquisition/source status, receipt link, receipt artifact/link, paid date, payer identity, unpaid years, amount due, reassessment/status notes | block until bottom-right receipt link is captured or the browser-workflow/source blocker is preserved |
| Miami-Dade Official Records / Clerk | Commercial API client by folio when `MIAMI_DADE_CLERK_AUTH_KEY` exists; route-level mocked API proof maps returned records into deed facts; live credentialed proof remains | folio, owner, address, OR book/page | source status, latest record/deed candidate, OR book/page, recorded/document date, parties, title friction | `commercial_api_key_required` without AuthKey; human review required even when API returns records |
| Probate/Civil/Family Court | Commercial API client by case number when `MIAMI_DADE_CLERK_AUTH_KEY` exists; route-level mocked API proof maps returned records into case/docket/affidavit facts; live credentialed proof remains | case number, estate name, owner/decedent | source status, case number, case status, docket refs, affidavit/document availability | `commercial_api_key_required` without AuthKey; `commercial_api_input_required` without case number; human review required |
| Marriage/death/obituary/vital review | Configurable workflow API hook and direct Browserbase Function invocation implemented through `OBITUARY_VITAL_WORKFLOW_URL` / equivalents or `OBITUARY_VITAL_BROWSERBASE_FUNCTION_ID`; mocked Browserbase route proof maps function output into DOB/DOD/obituary/vital facts; real controlled-browser/API endpoint proof remains | estate/owner/decedent names, address, folio, case number, county | source status, obituary link, DOB/DOD, marriage-license signal, obituary snapshot, death-certificate status, incarceration/deceased-indicator status | `workflow_required` and `VITAL_RECORDS_WORKFLOW_REQUIRED` until the workflow/function is configured; human review required even when facts return |
| IDI Core / skip trace | Source-run bucket plus approved IDI report import; paid/API proof requires configured vendor access and approval | owner/address/DOB/DOD, approved report text/attachment, contact-review decisions | imported or live-run report evidence, contact candidates, accepted/promoted contact review state | imported report evidence resolves only the report-import detail; contacts stay manual until accepted/promoted; paid-run approval stays blocked unless a live approved run or approval record exists |
...
The source-run proof now promotes the governed-source catalog into `detailChecks` on the proof ledger. The Doc Prep rail renders these detail rows for bottom-right receipt capture, voter records, professional licenses, business/address associations, social profiles, deceased-indicator cross-checks, door knock / field visit, neighbor research, code enforcement, and manual document requests. IDI and skip trace also render their own guardrail detail checks: access mode, paid-run approval, first-run lock, approved report import, contact review, provider access, and skip-trace contact review. Every detail check keeps `legalTemplateAutofillAllowed: false`. Blocking detail checks are counted as `blockingDetailCheckCount` / `unresolvedDetailCheckCount`, and `readyForOperatorReview` remains false while any blocking detail check is unresolved.
...
- IDI report import and IDI contact review are separate gates. Imported report evidence may resolve `idi_report_import`, but imported contacts cannot resolve `idi_contact_review` until accepted or promoted by an operator. Report import alone cannot resolve `idi_paid_run_approval`.
...
15. IDI report import into source-run plus contact-review split: imported reports count as report evidence, accepted/promoted contacts count as reviewed contact evidence, paid approval remains its own gate.
16. Tax/deed depth adapters.
17. Probate/heirship research queue.
18. Paid/manual source governance.
19. Completed lead report and offer math payload.

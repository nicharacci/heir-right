...
- Clerk API proof addendum: the artifact source-run contract now mocks configured Miami-Dade Clerk Commercial Data Services responses through the real `/api/discovery/external-source-run` handler. It verifies Official Records deed facts, probate/civil docket facts, affidavit-of-heirs evidence, detail-check resolution, and AuthKey redaction.
- Browserbase proof addendum: the artifact source-run contract now mocks configured Browserbase Function responses through the real `/api/discovery/external-source-run` handler. It verifies Tax Collector receipt/payer/date/amount/unpaid-year/reassessment facts, vital/obituary DOB/DOD/source facts, detail-check resolution, and API-key redaction.
...
- Clerk route-mapping proof: `pnpm --dir probate-lead-engine --filter @ple/artifact test` now includes `clerk_commercial_api_route_fact_mapping`, which proves configured Clerk API responses travel through `/api/discovery/external-source-run` into `latest_deed`, `civil_family_docket_ref`, `affidavit_of_heirs_status`, and related detail-check evidence while redacting the AuthKey from source URLs.
- Browserbase route-mapping proof: `pnpm --dir probate-lead-engine --filter @ple/artifact test` now includes `browserbase_source_run_route_fact_mapping`, which proves configured Tax Collector and vital/obituary Browserbase Function responses travel through `/api/discovery/external-source-run` into receipt, payer, tax amount, DOB/DOD, obituary, marriage, and death-certificate facts while keeping API keys out of stored source URLs.
